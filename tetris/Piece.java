...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		int[] skr = new int[this.width];
		for(int i = 0; i < skr.length; i++){
			skr[i] = height + 1;
			for(int j = 0; j < body.length; j++){
				if(body[j].x == i && points[j].y < skr[i]){
					skr[i] = body[j].y;
				}
			}
		}
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] rotatedBody = new TPoint[getBody().length];
		...
		for(int i = 0; i < getBody().length; i++){
			...
			rotatedBody[i] = new TPoint(getHeight() - curPtY - 1, curPtX);
		}
		
		Piece rotatedPiece = new Piece(rotatedBody);
